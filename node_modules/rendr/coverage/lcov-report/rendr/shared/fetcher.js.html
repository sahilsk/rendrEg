...
<html lang="en">
<head>
    <title>Code coverage report for rendr/shared/fetcher.js</title>
    <meta charset="utf-8">
...
<div class="header high">
    <h1>Code coverage report for <span class="entity">rendr/shared/fetcher.js</span></h1>
    <h2>
        
        Statements: <span class="metric">83.73% <small>(139 / 166)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
        
        Branches: <span class="metric">67.47% <small>(56 / 83)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
        
        Functions: <span class="metric">88% <small>(22 / 25)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
        
        Lines: <span class="metric">85.19% <small>(138 / 162)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
    </h2>
    <div class="path"><a href="../../index.html">All files</a> &#187; <a href="index.html">rendr/shared/</a> &#187; fetcher.js</div>
</div>
<div class="body">
...
&nbsp;
   ...
   */
  if (spec.params != null) {
    <span class="missing-if-branch" title="else path not taken"" >E</span>if (spec.model != null) {
      // If it's a model, merge the given params with the model attributes.
      _.defaults(attrsOrModels, spec.params);
    } else <span class="cstat-no" title="statement not covered" >if (spec.collection != null) {</span>
      // If it's a collection, merge the given params with the options.
<span class="cstat-no" title="statement not covered" >      _.defaults(options, spec.params);</span>
    }
  }
  _.defaults(options, {
    app: this.app
...
Fetcher.prototype.checkedFreshKey = function(spec) {
  var meta;
  meta = {
    name: spec.model || <span class="branch-1 cbranch-no" title="branch not covered" >spec.collection,</span>
    params: spec.params
  };
...
<span class="cstat-no" title="statement not covered" >          collectionData = this.collectionStore.get(spec.collection, spec.params);</span>
<span class="cstat-no" title="statement not covered" >          if (collectionData) {</span>
<span class="cstat-no" title="statement not covered" >            modelData = this.retrieveModelsForCollectionName(spec.collection, collectionData.ids);</span>
<span class="cstat-no" title="statement not covered" >            modelOptions = {</span>
...
&nbsp;
           ...
           */
          <span class="missing-if-branch" title="if path not taken"" >I</span>if (spec.checkFresh &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >!global.isServer </span>&amp;&amp; <span class="branch-2 cbranch-no" title="branch not covered" >this.shouldCheckFresh(spec))</span> {
<span class="cstat-no" title="statement not covered" >            model.checkFresh();</span>
<span class="cstat-no" title="statement not covered" >            this.didCheckFresh(spec);</span>
          }
          cb(null, model);
        } else {
          /**
...
Fetcher.prototype.needsFetch = function(modelData, spec) {
  <span class="missing-if-branch" title="if path not taken"" >I</span>if (modelData == null) <span class="cstat-no" title="statement not covered" >return true;</span>
  if (this.isMissingKeys(modelData, spec.ensureKeys)) return true;
  <span class="missing-if-branch" title="if path not taken"" >I</span>if (typeof spec.needsFetch === 'function' &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >spec.needsFetch(modelData))</span> <span class="cstat-no" title="statement not covered" >return true;</span>
  return false;
};
&nbsp;
...
Fetcher.prototype.fetchFromApi = function(spec, callback) {
  ...
  model.fetch({
    ...
    error: <span class="fstat-no" title="function not covered" >function(model, body, options) {</span>
<span class="cstat-no" title="statement not covered" >      var bodyOutput, err;</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >      bodyOutput = typeof body === 'string' ? body.slice(0, 150) : JSON.stringify(body);</span>
<span class="cstat-no" title="statement not covered" >      err = new Error("ERROR fetching model '" + modelUtils.modelName(model.constructor) + "' with options '" + JSON.stringify(options) + "'. Response: " + bodyOutput);</span>
...
&nbsp;
  ...
  } else <span class="missing-if-branch" title="else path not taken"" >E</span>if (modelUtils.isModel(modelOrCollection)) {
...
&nbsp;
Fetcher.prototype.bootstrapData = <span class="fstat-no" title="function not covered" >function(modelMap) {</span>
<span class="cstat-no" title="statement not covered" >  var results = {}</span>
    , modelOrCollection;
&nbsp;
<span class="cstat-no" title="statement not covered" >  _.each(modelMap, <span class="fstat-no" title="function not covered" >function(map, name) {</span></span>
<span class="cstat-no" title="statement not covered" >    modelOrCollection = this.getModelForSpec(map.summary, map.data, _.pick(map.summary, 'params', 'meta'));</span>
<span class="cstat-no" title="statement not covered" >    results[name] = modelOrCollection;</span>
  }, this);
<span class="cstat-no" title="statement not covered" >  this.storeResults(results);</span>
};
&nbsp;
...
&nbsp;
  ...
  _.each(summaries, function(summary, name) {
    if (summary.model != null) {
    ...
    } else <span class="missing-if-branch" title="else path not taken"" >E</span>if (summary.collection != null) {
      ...
      <span class="missing-if-branch" title="if path not taken"" >I</span>if (collectionData == null) {
<span class="cstat-no" title="statement not covered" >        throw new Error("Collection of type \"" + summary.collection + "\" not found for params: " + JSON.stringify(summary.params));</span>
      }
...
<span class="cstat-no" title="statement not covered" >    if (options.readFromCache == null) {</span>
<span class="cstat-no" title="statement not covered" >      options.readFromCache = true;</span>
    }
<span class="cstat-no" title="statement not covered" >    if (options.writeToCache == null) {</span>
<span class="cstat-no" title="statement not covered" >      options.writeToCache = true;</span>
    }
  }
&nbsp;
  this.pendingFetches++;
